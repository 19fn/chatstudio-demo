  ...
  it('saves validated settings without returning the API key', async () => {
    const repository = {
      save: vi.fn().mockResolvedValue({
        provider: 'azure-openai', endpoint: 'https://example.openai.azure.com', deploymentName: 'chat', modelId: 'gpt-5.4-mini', apiVersion: '2025-04-01-preview',
      }),
    ...
    const response = await request(createApp({ config, providerSettingsRepository: repository }))
      ...
      .send({
        provider: 'azure-openai', endpoint: 'https://example.openai.azure.com', apiKey: 'secret-api-key', deploymentName: 'chat', modelId: 'gpt-5.4-mini', apiVersion: '2025-04-01-preview',
      });

    expect(response.status).toBe(200);
    expect(response.body).toEqual({
      settings: { provider: 'azure-openai', endpoint: 'https://example.openai.azure.com', deploymentName: 'chat', modelId: 'gpt-5.4-mini', apiVersion: '2025-04-01-preview' },
    });
